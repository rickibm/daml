...
import com.digitalasset.platform.sandbox.stores.ledger.sql.SqlLedger.{noOfShortLivedConnections, noOfStreamingConnections}
import com.digitalasset.platform.sandbox.stores.ledger.sql.dao.{LedgerDao, PersistenceEntry, PostgresLedgerDao}
...
class PostgresIndexer(initialOffset: Long, beginAfter: Option[LedgerString], ledgerDao: LedgerDao)(implicit mat: Materializer) extends Indexer with AutoCloseable {
  ...
  /**
    ...
    */
  override def subscribe(readService: ReadService, onError: Throwable => Unit, onComplete: () => Unit): Future[IndexFeedHandle] = {
  ...
  private def handleStateUpdate(offset: Offset, update: Update): Future[Unit] = {
    val future = update match {
      case Heartbeat(recordTime) =>
        ledgerDao.storeLedgerEntry(headRef, headRef + 1, Some(offset.toLedgerString), PersistenceEntry.Checkpoint(LedgerEntry.Checkpoint(recordTime))).map(_ => ())(DEC)

      case PartyAddedToParticipant(party) =>
        ledgerDao.storeParty(party, None).map(_ => ())(DEC)

      case PublicPackageUploaded(archive) =>
        // TODO (GS) implement once https://github.com/digital-asset/daml/pull/1610 lands
        Future.successful(())

      case TransactionAccepted(optSubmitterInfo, transactionMeta, transaction, transactionId, recordTime, referencedContracts) =>
        ...
        val mappedLocalImplicitDisclosure = blindingInfo.localImplicitDisclosure.map {
          case (nodeId, parties) => SandboxEventIdFormatter.fromTransactionId(transactionId, nodeId) -> parties
        }
        ...
        val pt = PersistenceEntry.Transaction(
          LedgerEntry.Transaction(
            ...
            recordTime,
            transaction.mapNodeId(_.name),
            mappedDisclosure
          ),
          mappedLocalImplicitDisclosure,
          blindingInfo.globalImplicitDisclosure
        )
        ledgerDao.storeLedgerEntry(headRef, headRef + 1, Some(offset.toLedgerString), pt).map(_ => ())(DEC)
      ...
      case CommandRejected(submitterInfo, reason) =>
        ...
        ledgerDao.storeLedgerEntry(headRef, headRef + 1, Some(offset.toLedgerString), rejection).map(_ => ())(DEC)
    }
    future.map(_ => headRef = headRef + 1)(DEC)
  }



  private def toDomainRejection(submitterInfo: SubmitterInfo, state: RejectionReason): domain.RejectionReason = state match {
    case RejectionReason.Inconsistent => domain.RejectionReason.Inconsistent(RejectionReason.Inconsistent.description)
    case RejectionReason.Disputed(reason @ _) => domain.RejectionReason.Disputed(state.description)
    case RejectionReason.DuplicateCommand => domain.RejectionReason.DuplicateCommandId(state.description)
    ...
    case RejectionReason.PartyNotKnownOnLedger => domain.RejectionReason.PartyNotKnownOnLedger(state.description)
    case RejectionReason.SubmitterCannotActViaParticipant(details) => domain.RejectionReason.SubmitterCannotActViaParticipant(state.description)
  }

  private def indexHandleFromKillSwitch(ks: KillSwitch, completionFuture: Future[Done]): IndexFeedHandle = new IndexFeedHandle {
