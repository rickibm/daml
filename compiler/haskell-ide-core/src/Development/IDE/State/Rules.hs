...
module Development.IDE.State.Rules(
    ...
    Priority(..),
    runAction, runActions, use_, uses_, actionToMaybe,
...
getGhcCore file = actionToMaybe $ do
    files <- transitiveModuleDeps <$> use_ GetDependencies file
    pms   <- uses_ GetParsedModule $ files ++ [file]
    cores <- uses_ GenerateCore $ map fileFromParsedModule pms
...
getAtPoint file pos = fmap join $ actionToMaybe $ do
  files <- transitiveModuleDeps <$> use_ GetDependencies file
  tms   <- uses_ TypeCheck (file : files)
...
getDefinition file pos = fmap join $ actionToMaybe $ do
    spans <- use_ GetSpanInfo file
    pkgState <- use_ GhcSession ""
    opts <- getOpts
    AtPoint.gotoDefinition opts pkgState spans pos
