...
resource "google_compute_firewall" "temp-hoogle-ssh" {
  name        = "hoogle-firewall-ssh"
  network     = "${google_compute_network.hoogle.name}"
  target_tags = ["hoogle"]
...
resource "google_compute_target_http_proxy" "hoogle-https" {
  name    = "hoogle-https"
...
resource "google_compute_global_forwarding_rule" "hoogle_https" {
  name       = "hoogle-https"
  target     = "${google_compute_target_http_proxy.hoogle-https.self_link}"
  ip_address = "${google_compute_global_address.hoogle.address}"
  port_range = "8080"
}
